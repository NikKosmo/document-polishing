...
class DocumentPolisher:
    ...
    def _create_ambiguity_detector(self) -> AmbiguityDetector:
        ...
        if self.judge_model in self.model_manager.list_available():
            ...
            return AmbiguityDetector(strategy="llm_judge", llm_query_func=self._create_judge_query_func())
        else:
    ...
    def polish(self, models: list = None, profile: str = None):
        ...
        if self.model_manager.sessions_enabled():
            ...
            session_init_step = SessionInitStep(self.config["models"], self.session_config)
            session_init_result = session_init_step.init_sessions(
                extraction_result.document_content, models, purpose_prompt
            )
        ...
        try:
            ...
            self._log(error_msg)
            self._log("Judge failure - aborting polish process")
...
def main():
    parser = argparse.ArgumentParser(
        description="Documentation Polishing System", formatter_class=argparse.RawDescriptionHelpFormatter
    )
