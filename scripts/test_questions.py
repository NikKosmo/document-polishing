...
from questioning_step import Question, QuestioningStep
...
def create_session_manager(document_content: str, models: list, purpose: str = None) -> SessionManager:
    ...
    models_config = {}
    for model in models:
        models_config[model] = {"command": model, "timeout": 30}

    # Create session config
    session_config = {"enabled": True, "mode": "continue-on-error", "max_retries": 2}
    ...
    session_result = session_init_step.init_sessions(
        document_content=document_content, model_names=models, purpose_prompt=purpose
    )
...
def cmd_generate(args):
    ...
    step = QuestioningStep(enable_document_level=args.document_level)

    # Generate questions
    result = step.generate_questions(sections=sections, document_text=document_text, document_path=args.document)
    ...
    print(
        f"  Coverage: {result.statistics['coverage']['section_coverage_pct']:.1f}% sections, "
...
def cmd_test(args):
    ...
    answers = step.collect_answers(questions=questions, models=models, session_manager=session_manager)
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        for qid, model_answers in answers.items():
            serializable_answers[qid] = {model: answer.to_dict() for model, answer in model_answers.items()}
        json.dump(
            {"questions": data.get("questions", []), "answers": serializable_answers}, f, indent=2, ensure_ascii=False
        )
...
def cmd_evaluate(args):
    ...
    for qid, model_answers in answers_data.items():
        answers[qid] = {model: QuestionAnswer.from_dict(ans) for model, ans in model_answers.items()}

    # Evaluate answers
    print(f"\nEvaluating {len(questions)} questions...")
    results = step.evaluate_answers(
        questions=questions, answers=answers, sections=sections, session_manager=session_manager, judge_model=args.judge
    )
...
""",
    ...
    gen_parser.add_argument(
        "--document-level", action="store_true", default=False, help="Include document-level questions (default: False)"
    )
    gen_parser.add_argument(
        "--no-document-level", dest="document_level", action="store_false", help="Disable document-level questions"
    )
    ...
    test_parser.add_argument(
        "--session", required=False, help="Path to session_metadata.json (optional, will create new if not provided)"
    )
